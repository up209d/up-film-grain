...
breaks their whole purpose. Read alongside `docs/film-texture/placement.md`,
...
* `global_*` (step 9) is after every mask, so it reaches the regions the masks
  protect. Fold it in earlier and it becomes just more masked grain. **Below
  Film Texture as of 2026-08-09, on request**, which puts it below Halation and
  Tone Response as well: the layer is no longer bloomed by the halo and no
  longer developed by the characteristic curve, and its four source-masked
  envelopes read a frame that has the dust, the hair and the leaks in it. That
  is consistent with what the layer models — the print stock's grain and the
  scan's noise, generated by a pass that sees the debris — and it is a change of
  look, not a refactor. See `docs/film-texture/placement.md`.
* `sharpen` (step 10) is dead last, **after** the grain stages, because the
